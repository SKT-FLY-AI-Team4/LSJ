...
class NaviyamNLU:
    ...
    def _build_intent_patterns(self) -> List[IntentPattern]:
        ...
        patterns = [
            ...
            IntentPattern(
                ...
                keywords=['먹고싶어', '추천', '맛집', '뭐먹', '음식', '메뉴', '치킨', '피자', '햄버거', '파스타', '한식', '중식', '일식', '양식', '분식', '김치찌개', '된장찌개', '라면', '떡볶이', '짜장면', '짬뽕', '탕수육', '초밥', '돈까스', '카레'],
                patterns=[
                    r'.*(먹고\s*싶|드시고\s*싶).*',
                    r'.*(추천|소개).*',
                    r'.*(뭐\s*먹|무엇.*먹).*',
                    r'.*(맛집|맛있는.*곳).*',
                    r'.*(메뉴.*있|있.*메뉴).*',
                    r'.*(치킨|피자|햄버거|파스타|한식|중식|일식|양식|분식).*',
    ...
    def _build_entity_patterns(self) -> Dict[str, List[str]]:
        ...
        return {
            'food_types': [
                r'(한식|중식|일식|양식|분식|치킨|피자|햄버거|파스타)',
