...
"""
데이터 로더 최종 수정
RAG 시스템과 동일한 데이터 소스(test_data.json) 사용하도록 수정
"""
...
def fix_data_loader_architecture():
    """데이터 로더 아키텍처 수정 - RAG와 동일한 데이터 소스 사용"""
    
    print("=== 나비얌 데이터 로더 최종 수정 ===")
    
    # 1. 기존 캐시 삭제
    cache_dir = Path("cache")
    if cache_dir.exists():
        for file in cache_dir.glob("*.json"):
            print(f"기존 캐시 삭제: {file}")
            file.unlink()
    
    # 2. data_loader.py 백업 및 간단화
    data_loader_path = Path("data/data_loader.py")
    backup_path = Path("data/data_loader_original.py")
    
    if data_loader_path.exists() and not backup_path.exists():
        shutil.copy2(data_loader_path, backup_path)
        print(f"기존 데이터 로더 백업: {backup_path}")
    
    # 3. 간단한 데이터 로더 작성 (RAG 데이터 직접 사용)
    simple_loader_content = '''"""
나비얌 데이터 로더 (간단화 버전)
...
'''
    ...
    print(f"데이터 로더 수정 완료: {data_loader_path}")
    ...
    if rag_data_file.exists():
        ...
        print("RAG 데이터 파일이 없습니다!")
        return False
    
    print("=== 수정 완료 ===")
    print("이제 DataLoader와 RAG가 동일한 데이터를 사용합니다!")
    print("'python main.py --mode chat --use_4bit' 실행해보세요!")
    
    return True

if __name__ == "__main__":
    success = fix_data_loader_architecture()
    if success:
        print("\n데이터 아키텍처 통합 완료!")
    else:
        print("\n수정 실패.")